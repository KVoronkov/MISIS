...
def transfer_money(accounts, account_from, account_to, value):
    ...
    Потенциальные ошибки:
        - строка 53, 54 - если там неправильный тип данных, добавление может не сработать
    """
...
def transaction(accounts, account_from, account_to, value) -> None:
    if accounts[account_from] < value:
        raise NoMoneyToWithdrawError(
            f'На счету {account_from} не хватает денег для перевода')
    if value <= 0:
        raise PaymentError('Ошибка при переводе: отрицательная сумма')
    try:
